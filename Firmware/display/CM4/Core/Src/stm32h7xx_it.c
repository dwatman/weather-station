...
void SysTick_Handler(void)
{
  /* USER CODE BEGIN SysTick_IRQn 0 */
	...
	// Regular ticks (ideally not simultaneous)
	if (count % 501 == 0) flag_update = 1;
	if (count % 5001 == 0) flag_status = 1;
...
void DMA1_Stream1_IRQHandler(void)
{
  /* USER CODE BEGIN DMA1_Stream1_IRQn 0 */
	IR_Decoder_DMA_IRQHandler(&ir_decode);
...
void TIM1_UP_IRQHandler(void)
{
  /* USER CODE BEGIN TIM1_UP_IRQn 0 */
	IR_TIM1_UP_IRQHandler(&ir_decode);  // increment overflow_count, set error if >=2
...
void TIM1_CC_IRQHandler(void)
{
  /* USER CODE BEGIN TIM1_CC_IRQn 0 */
	IR_TIM1_CC_IRQHandler(&ir_decode);  // reset overflow_count
